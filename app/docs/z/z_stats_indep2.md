# 基于原始数据的独立样本Z检验 (Independent Samples Z-Test Based on Raw Data)

## 概述
基于原始数据的独立样本Z检验用于检验两个独立样本的均数是否有显著性差异。该检验适用于两组数据相互独立，总体标准差已知且仅有原始数据的情况，模块将自动计算样本统计量。
...
## 示例
比较两组患者（男性和女性）的收缩压是否有显著性差异，每组提供原始数据列表。