# 基于原始数据的单样本t检验 (Single Sample T-Test Based on Raw Data)

## 概述
基于原始数据的单样本t检验用于检验样本均数与已知总体均数之间是否存在显著性差异。该检验适用于仅有原始数据的情况，模块将自动计算样本统计量。
...
## 示例
给定总体均数为100和一组原始数据，通过单样本t检验判断样本均数与总体均数的差异。