...
临床两独立样本Mann-Whitney U检验统计分析模块提供全面的两独立样本Mann-Whitney U检验统计分析功能，用于临床数据中两个独立样本的比较，是一种重要的非参数统计分析方法。Mann-Whitney U检验（等价于Wilcoxon秩和检验）通过比较两个独立样本的秩次，来判断两组之间是否存在显著差异，广泛应用于医学研究中的两组比较、非正态数据检验、等级资料分析等领域。
...
### Mann-Whitney U检验函数 - mann_whitney_u_test
...
### Wilcoxon秩和检验函数 - wilcoxon_rank_sum_test
...
### 结果计算函数 - cal_result_rs_indep

生成两独立样本Mann-Whitney U检验统计分析的完整报告字典。此函数整合了两独立样本Mann-Whitney U检验的所有关键指标，生成标准化的字典格式报告，适用于临床研究报告的需求，提供全面的检验结果。
...
**返回值**：
- 包含两独立样本Mann-Whitney U检验统计分析指标的字典，包括：
...
# 执行两独立样本Mann-Whitney U检验
result = cal_result_rs_indep(group1_data, group2_data)
