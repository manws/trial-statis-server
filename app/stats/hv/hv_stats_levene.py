...
本模块提供全面的Levene方差齐性检验统计分析功能，用于临床数据中多组方差齐性的检验，
是检验多组数据方差是否相等的经典统计方法。Levene检验在医学研究中广泛应用，
如比较多个治疗组方差的齐性、验证方差分析的前提条件等。相比Bartlett检验，
Levene检验对数据正态性假设不敏感，更加稳健。

【模块功能概述】:
1. Levene统计量计算：计算多组数据方差齐性的Levene统计量
2. 绝对偏差计算：计算各组数据相对于其中心的绝对偏差
...
- "我的多组数据适合用Levene检验比较方差吗？"
- "如何解释Levene检验的结果？"
- "P值小于0.05意味着什么？"
- "Levene检验的前提条件是什么？"
- "Levene检验和Bartlett检验有何区别？"
...
from app.schemas.request_data.hv_param import HVParamLevene

def calculate_levene_test_variance_homogeneity(data_list: List[List[float]]) -> Dict[str, Any]:
    """
    执行Levene方差齐性检验
    
    在临床研究中，这用于检验多组数据的方差是否齐性，
    即比较多个组数据的变异程度是否存在显著差异。
    相比Bartlett检验，Levene检验对数据正态性假设不敏感，更加稳健。
    
    Args:
        data_list: List[List[float]]，每个子列表代表一组数据
        
    Returns:
        Dict[str, Any]: 包含Levene检验统计量和结果的字典
    ...
    if not data_list or len(data_list) < 2:
        raise ValueError("Levene检验至少需要2组数据")
    ...
    for data in groups_data:
        center = np.median(data)  # 使用中位数作为中心
    ...
    return {
        "input_parameters": {
            ...
            "group_centers": [float(center) for center in group_centers]
        },
        "test_statistics": {
        ...
        "interpretation": {
            ...
            "assumption_note": "注意：Levene检验使用组内中位数作为中心，对偏离正态分布的数据较为稳健"
        }
    }
...
def cal_result_hv_levene(param: HVParamLevene) -> Dict[str, Any]:
    """
    生成Levene方差齐性检验统计分析的完整报告字典
    
    此函数整合了Levene方差齐性检验的所有关键指标，生成标准化的字典格式报告，
    适用于临床研究报告的需求，提供全面的Levene检验结果。报告包括输入参数、
    检验统计量、显著性检验和统计解释等信息，
    便于临床医生和研究人员快速理解Levene检验的特征。
    
    Args:
        data_list: List[List[float]]，每个子列表代表一组数据
    
    Returns:
        Dict[str, Any]: 包含Levene方差齐性检验统计分析指标的字典，键为指标名称，值为对应的统计量
    ...
    results = calculate_levene_test_variance_homogeneity(data_list)
    
    # 构建结果字典
    result_dict = {
        "table_name": "Levene方差齐性检验分析",
