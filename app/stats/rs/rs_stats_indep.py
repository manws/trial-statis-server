"""
临床两独立样本Mann-Whitney U检验统计分析模块 (Clinical Two Independent Samples Mann-Whitney U Test Statistics Module)

本模块提供全面的两独立样本Mann-Whitney U检验统计分析功能，用于临床数据中两个独立样本的比较，
是一种重要的非参数统计分析方法。Mann-Whitney U检验（等价于Wilcoxon秩和检验）
通过比较两个独立样本的秩次，来判断两组之间是否存在显著差异，广泛应用于医学研究中的
两组比较、非正态数据检验、等级资料分析等领域。
...
- "我的数据适合用Mann-Whitney U检验吗？"
- "如何解释Mann-Whitney U检验的结果？"
- "Mann-Whitney U检验的前提条件是什么？"
- "如何判断差异是否具有统计学意义？"
...
def mann_whitney_u_test(group1_data: List[float], group2_data: List[float]) -> Dict:
    """
    ...
    # 计算U统计量
    # U1 = n1*n2 + n1*(n1+1)/2 - R1 (R1是第一组的秩和)
    # U2 = n1*n2 + n2*(n2+1)/2 - R2 (R2是第二组的秩和)
    R1 = sum(group1_ranks)
    R2 = sum(group2_ranks)
    ...
    return {
        ...
        "interpretation": {
            "mann_whitney_interpretation": f"Mann-Whitney U检验{'不' if p_value_two_sided >= 0.05 else ''}显著 (p={'≥' if p_value_two_sided >= 0.05 else '<'}0.05)",
            "mann_whitney_interpretation_01": f"在0.01水平下{'不' if p_value_two_sided >= 0.01 else ''}显著 (p={'≥' if p_value_two_sided >= 0.01 else '<'}0.01)"
        }
    }


def wilcoxon_rank_sum_test(group1_data: List[float], group2_data: List[float]) -> Dict:
    """
    Wilcoxon秩和检验（等价于Mann-Whitney U检验）
    ...
    - 包含检验统计量和结果的字典
    """
    # Wilcoxon秩和检验与Mann-Whitney U检验在数学上是等价的
    # 这里我们使用相同的计算方法，但提供不同的解释
...
def cal_result_rs_indep(param: RSParamIndep) -> Dict:
    ...
    此函数整合了两独立样本Mann-Whitney U检验的所有关键指标，生成标准化的字典格式报告，
    适用于临床研究报告的需求，提供全面的检验结果。报告包括输入参数、
    检验统计量和统计解释等信息，
    便于临床医生和研究人员快速理解两独立样本Mann-Whitney U检验的特征。
    ...
    Returns:
        Dict: 包含两独立样本Mann-Whitney U检验统计分析指标的字典，键为指标名称，值为对应的统计量
    ...
    # 构建结果字典
    result_dict = {
        "table_name": "两独立样本Mann-Whitney U检验分析",
